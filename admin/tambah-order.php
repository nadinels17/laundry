...
$qry = mysqli_query($konek, "SELECT * FROM tb_orderan JOIN tb_customer ON 
                                tb_orderan.id_member = tb_customer.id_member");


if(isset($_POST["kirim"])){
    if(tambahorder($_POST) >0){
        echo "<script>
        alert('Berhasil Menambahkan Pemesanan')
        document.location.href = 'data-orderan.php';
        </script>";
    }else {
        echo "<script>
        alert('Gagal Menambahkan Pemesanan')
        document.location.href = 'tambah-orderan.php';
        </script>";
    }
}
